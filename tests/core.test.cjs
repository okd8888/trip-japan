...
test('Worker 檢查授權、衝突、私人欄位與讀取',async()=>{
  ...
  const created=await worker.fetch(new Request('https://worker.test/api/trip',{method:'POST',body:JSON.stringify({trip})}),env);
  assert.equal(created.status,201);
  const {code,editKey}=await created.json();
  const put=(key,version)=>new Request(`https://worker.test/api/trip/${code}`,{method:'PUT',headers:{'x-edit-key':key},body:JSON.stringify({version,trip})});
  assert.equal((await worker.fetch(put('bad',1),env)).status,403);
  const simultaneous=await Promise.all([worker.fetch(put(editKey,1),env),worker.fetch(put(editKey,1),env)]);
  assert.deepEqual(simultaneous.map(r=>r.status).sort(),[200,409]);
  assert.equal((await worker.fetch(put(editKey,1),env)).status,409);
  ...
  const expense=new Request(`https://worker.test/api/expenses/${code}`,{method:'POST',headers:{'x-edit-key':editKey},body:JSON.stringify({expenses:[{id:'e1',category:'停車',amount:500,updatedAt:Date.now()}]})});
