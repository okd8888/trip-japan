...
公開網站免登入即可建立、修改自己的旅程並依目的地自動排程，資料各自儲存在瀏覽器。分享連結維持唯讀；雲端管理功能採自訂 Token 登入。設定、部署與權限驗證請先看 [管理員 Token 指南](docs/ADMIN-TOKEN.md)；舊版編輯金鑰不再授予雲端寫入權限。
...
```
index.html            主介面與原有樣式
companion.css         旅行模式與手機／桌面版配置
companion-core.js     時間、狀態、導航與公開資料處理
companion.js          下一站、自駕資訊、天氣與旅行操作
...
**方法 A（推薦）：在網頁上改。** 打開網站 → 底部「更多」→「編輯行程／備份・設定目的地與自動排程」，不需要登入，全部都是表單。每個瀏覽器保留一份個人旅程，換裝置或清除瀏覽資料前，請從「編輯行程／備份」匯出備份。收到分享連結時，可按「建立自己的旅程」回到個人編輯頁：
...
- 個人旅程免登入在本機編輯；分享行程維持唯讀，雲端寫入需要正確管理員 Token。
