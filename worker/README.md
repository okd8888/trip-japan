...
> **新版部署入口：[管理員 Token 與部署](../docs/ADMIN-TOKEN.md)。**
> 以下內容保留作為 v2.0 舊版後端參考。新版寫入只接受登入工作階段，舊編輯金鑰不再有效；需用 Wrangler 打包多個檔案與管理介面，不能照舊版方式只貼 `index.js`。本次仍沿用既有 D1 行程與花費資料。
...
**編輯金鑰只存在你這台裝置的瀏覽器**（伺服器只留不可逆的雜湊），清掉瀏覽資料就拿不回來。
按「**複製編輯金鑰**」複製一份存到別的地方，或勾「顯示金鑰」看明碼。
金鑰欄位是遮蔽的，不要手動改裡面的字——改了就對不上，會變成唯讀。
